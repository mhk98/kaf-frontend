...
import { unwrapApiData, unwrapApiList } from "@/lib/apiData";
...
function mapToProduct(item: ApiProduct): Product {
  ...
  return {
    ...
    bestDeals: toBoolean(item.bestDeals),
...
export async function fetchProductById(id: number): Promise<Product | null> {
  try {
    ...
    const item = unwrapApiData<ApiProduct | null>(json, null);
    return item ? mapToProduct(item) : null;
  } catch {
    return null;
  }
}
...
export async function fetchStorefrontProducts(
  ...
  let products = unwrapApiList<ApiProduct>(res).map(mapToProduct);
