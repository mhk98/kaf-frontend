...
function CategorySectionHeader({
  ...
  return (
    <header
      className="fl-category-section-header"
      style={{ borderColor: accent }}
    >
      <span className="fl-category-section-index" style={{ color: accent }}>
      ...
      <Link
        href={`/?menu=${encodeURIComponent(category.label)}`}
        className="fl-category-section-link"
...
function ProductTile({
  ...
  return (
    <Link
      ...
      className={`fl-product group ${large ? "fl-product-large" : ""}`}
    >
      ...
      <div className="fl-product-price">
        ...
        {product.originalPrice > product.discountedPrice && (
          <del>৳ {product.originalPrice.toLocaleString("en-US")}</del>
        )}
      </div>
    </Link>
  );
...
function EditorialGroup({
  ...
  return (
    <section
      className={`fl-editorial ${reverse ? "fl-editorial-reverse" : ""}`}
    >
      <Link
        href={`/?menu=${encodeURIComponent(category.label)}`}
        className="fl-editorial-cover group"
      >
        <Image
...
export default function FabrilifeHomepage({
  products,
  categories,
  banners,
  settings,
  ...
  const defaultCategory =
    categories.find((category) => /^men$/i.test(category.label))?.label ||
    categories[0]?.label ||
    products.find((product) => product.category)?.category ||
    "Men";
  const promos = banners.filter((banner) =>
    /home promo/i.test(banner.category || ""),
  );
  const stories = banners.filter((banner) =>
    /home story/i.test(banner.category || ""),
  );
  const wideBanners = banners.filter((banner) =>
    /home (app|affiliate|bulk)/i.test(banner.category || ""),
  );
  ...
  const megaDealProducts = products
    .filter(
      (product) =>
        product.discount > 0 || product.originalPrice > product.discountedPrice,
    )
    .slice(0, 12);
  const bulkBanner = wideBanners.find((banner) =>
    /bulk/i.test(banner.category || ""),
  );
  const affiliateBanner = wideBanners.find((banner) =>
    /affiliate|app/i.test(banner.category || ""),
  );
  ...
  return (
    <div className="fl-home-shell">
      ...
      <section id="new-arrival" className="fl-new-arrival">
        <h1>
          <Link
            href={`/?menu=${encodeURIComponent(defaultCategory)}&offer=new`}
          >
            New Arrival
          </Link>
        </h1>
      ...
      {topSellingProducts.length > 0 && (
        <section id="top-selling" className="fl-top-selling">
          <h2>
            <Link
              href={`/?menu=${encodeURIComponent(defaultCategory)}&offer=top`}
            >
      ...
      {products.some((product) => product.freeShipping) && (
        <section id="free-delivery" className="fl-top-selling">
          <h2>
            <Link
              href={`/?menu=${encodeURIComponent(defaultCategory)}&offer=delivery`}
            >
              Free Delivery
            </Link>
          </h2>
      ...
      {megaDealProducts.length > 0 && (
        <section id="mega-deal" className="fl-top-selling">
          <h2>
            <Link
              href={`/?menu=${encodeURIComponent(defaultCategory)}&offer=deal`}
            >
              Mega Deal
            </Link>
          </h2>
          <div className="fl-new-grid">
            {megaDealProducts.map((product) => (
              <ProductTile key={`mega-deal-${product.id}`} product={product} />
            ))}
          </div>
        </section>
      )}
        ...
        <section className="fl-promo-grid">
          ...
          >
            {promos.map((banner) => (
              <a
                key={banner.Id}
                href={banner.linkUrl || "#new-arrival"}
                className="fl-promo-card"
              >
                <Image
                  src={banner.file}
                  alt={banner.alt}
      ...
      <section className="fl-category-mosaic">
        {categoryBatches[0]?.map(
          (group) =>
            group.category.imageUrl && (
              <Link
                key={group.category.Id}
                href={`/?menu=${encodeURIComponent(group.category.label)}`}
                className="fl-category-tile group"
              >
                <Image
                  src={group.category.imageUrl}
      ...
      <section className="fl-story">
        <div>
          <h2>
            {settings.metaTitle || "KAF LifeStyle"} <span>›</span>
          </h2>
          <h3>Because comfort and confidence go hand in hand.</h3>
          <p>
            {settings.metaDescription ||
              "Thoughtfully selected clothing, dependable quality and comfortable fits made for everyday confidence."}
          </p>
        </div>
        {stories.length > 0 && (
      ...
      <section id="lower-showcase" className="fl-lower-home">
        {lowerGroups.map((group, index) => (
          <div key={`lower-${group.category.Id}`}>
            ...
            />
            {index === 0 && (
              <div className="fl-socks-banner">
                <small>THE BEST EVERYDAY ESSENTIALS FOR BANGLADESH</small>
                <strong>Premium Antibacterial Socks</strong>
            ...
            {(index === 1 || index === 2) && (
              <div className="fl-feature-trio compact">
                {products
                  .slice(index === 1 ? 30 : 33, index === 1 ? 33 : 36)
                  .map((product) => (
                    <ProductTile
                      key={`lifestyle-${product.id}`}
                      product={product}
                      large
                    />
                  ))}
        ...
        <section className="fl-bulk-order">
          <div>
            ...
            <p>
              We provide quality apparel for custom branding, teams and
              organizations. Contact us for dependable bulk production and
              delivery.
            </p>
          </div>
          {bulkBanner && (
            <div className="fl-bulk-image">
              <Image
                src={bulkBanner.file}
                alt={bulkBanner.alt}
                fill
        ...
        {clients.length > 0 && (
          <section className="fl-partners">
            ...
            <p>
              We collaborate with trusted partners, brands, and businesses to
              deliver quality products and create meaningful experiences for our
              customers.
            </p>
            <div className="fl-partner-carousel">
              <HorizontalCarousel
                itemWidthClass="w-[calc((100%-24px)/2)] md:w-[calc((100%-120px)/6)]"
                gap={24}
                autoplay={clients.length > 2}
                interval={3500}
                showArrows={clients.length > 2}
              >
                {clients.map((client) => (
                  <a
                    key={client.Id}
        ...
        {affiliateBanner && (
          <a
            href={affiliateBanner.linkUrl || "#"}
            className="fl-affiliate-banner"
          >
            <Image
              src={affiliateBanner.file}
              alt={affiliateBanner.alt}
              fill
              sizes="100vw"
              className="object-cover"
              unoptimized
