...
export default function KafFooter({ settings }: { settings?: Partial<SiteSetting> | null }) {
  ...
  return <footer className="kaf-footer"><div className="kaf-footer-main"><div className="kaf-footer-grid">
    ...
    <section className="kaf-footer-follow"><Heading>{footer.socialLinksTitle || "FOLLOW US"}</Heading><p>Stay updated on our latest arrivals, exclusive promotions and events.</p><div className="kaf-footer-socials">{socials.map((social) => <a key={`${social.name}-${social.url}`} href={social.url} target={social.url === "#" ? undefined : "_blank"} rel={social.url === "#" ? undefined : "noreferrer"} aria-label={social.name} title={social.name} style={{ color: social.color }}><SocialIcon platform={social.name} /></a>)}</div>{facebook && <a href={facebook.url} target={facebook.url === "#" ? undefined : "_blank"} rel={facebook.url === "#" ? undefined : "noreferrer"} className="kaf-facebook-card"><i style={{ color: facebook.color }}><SocialIcon platform="Facebook" /></i><strong>KAF Lifestyle<small>Official Facebook page</small></strong><b>Follow</b></a>}<div className="kaf-app-badges"><Image src="/images/app-store-badges.jpg" alt="Download KAF Lifestyle on Google Play and the App Store" fill sizes="330px" className="object-contain object-left" /></div></section>
