...
function SocialIcon({ platform }: { platform: string }) {
  ...
  if (name.includes("facebook")) return <svg viewBox="0 0 24 24" aria-hidden="true"><path d="M13.7 21v-8h2.7l.4-3.1h-3.1v-2c0-.9.3-1.5 1.6-1.5H17V3.6c-.3 0-1.3-.1-2.5-.1-2.5 0-4.2 1.5-4.2 4.3v2.1H7.5V13h2.8v8h3.4Z" /></svg>;
  if (name.includes("whatsapp")) return <svg viewBox="0 0 24 24" aria-hidden="true"><path d="M19.1 4.9A9.9 9.9 0 0 0 3.5 16.8L2.1 22l5.3-1.4A9.9 9.9 0 0 0 22 11.9c0-2.6-1-5.1-2.9-7Zm-7 15a8 8 0 0 1-4.1-1.1l-.3-.2-3.1.8.8-3-.2-.3a8 8 0 1 1 6.9 3.8Zm4.4-6c-.2-.1-1.4-.7-1.7-.8-.2-.1-.4-.1-.6.1l-.8 1c-.1.2-.3.2-.5.1a6.5 6.5 0 0 1-3.2-2.8c-.2-.3 0-.4.1-.5l.4-.5.2-.4c.1-.2 0-.4 0-.5l-.8-2c-.2-.5-.4-.4-.6-.4h-.5c-.2 0-.5.1-.7.3-.2.3-1 1-1 2.4s1 2.8 1.2 3c.1.2 2 3.1 4.9 4.3.7.3 1.2.5 1.6.6.7.2 1.3.2 1.8.1.6-.1 1.4-.6 1.6-1.1.2-.6.2-1 .2-1.1 0-.2-.2-.3-.4-.4Z" /></svg>;
  if (name.includes("instagram")) return <svg viewBox="0 0 24 24" aria-hidden="true"><path d="M7.2 2h9.6A5.2 5.2 0 0 1 22 7.2v9.6a5.2 5.2 0 0 1-5.2 5.2H7.2A5.2 5.2 0 0 1 2 16.8V7.2A5.2 5.2 0 0 1 7.2 2Zm-.1 2A3.1 3.1 0 0 0 4 7.1v9.8A3.1 3.1 0 0 0 7.1 20h9.8a3.1 3.1 0 0 0 3.1-3.1V7.1A3.1 3.1 0 0 0 16.9 4H7.1Zm10.2 1.5a1.2 1.2 0 1 1 0 2.4 1.2 1.2 0 0 1 0-2.4ZM12 6.9a5.1 5.1 0 1 1 0 10.2 5.1 5.1 0 0 1 0-10.2Zm0 2a3.1 3.1 0 1 0 0 6.2 3.1 3.1 0 0 0 0-6.2Z" /></svg>;
  if (name.includes("tiktok")) return <svg viewBox="0 0 24 24" aria-hidden="true"><path d="M15.6 2c.3 2.3 1.6 3.7 3.9 3.9v3.2a8.6 8.6 0 0 1-3.9-1v6.2a6.3 6.3 0 1 1-5.4-6.2v3.3a3.1 3.1 0 1 0 2.2 3V2h3.2Z" /></svg>;
  if (name.includes("youtube")) return <svg viewBox="0 0 24 24" aria-hidden="true"><path d="M23.5 6.2a3 3 0 0 0-2.1-2.1C19.5 3.6 12 3.6 12 3.6s-7.5 0-9.4.5A3 3 0 0 0 .5 6.2 31 31 0 0 0 0 12a31 31 0 0 0 .5 5.8 3 3 0 0 0 2.1 2.1c1.9.5 9.4.5 9.4.5s7.5 0 9.4-.5a3 3 0 0 0 2.1-2.1A31 31 0 0 0 24 12a31 31 0 0 0-.5-5.8ZM9.6 15.6V8.4l6.3 3.6-6.3 3.6Z" /></svg>;
  return <svg viewBox="0 0 24 24" aria-hidden="true"><path d="M5.3 7.9H1.8V21h3.5V7.9ZM3.5 2A2 2 0 1 0 3.5 6a2 2 0 0 0 0-4Zm17.7 5.6c-2.3 0-3.4 1.3-3.9 2.2V7.9H8.8V21h3.5v-6.5c0-1.7.3-3.4 2.5-3.4 2.2 0 2.2 2 2.2 3.5V21h3.5v-7.5c0-4-2.1-5.9-5-5.9Z" /></svg>;
}
...
export default function KafFooter({ settings }: { settings?: Partial<SiteSetting> | null }) {
  ...
  const socials = SOCIALS.map((entry) => {
    ...
    return { ...entry, name: configured?.label || entry.name, url: configured?.url || String(s[entry.key] || fallbackUrl) };
  });
  ...
  return <footer className="kaf-footer"><div className="kaf-footer-main"><div className="kaf-footer-grid">
    ...
    <section className="kaf-footer-follow"><Heading>{footer.socialLinksTitle || "FOLLOW US"}</Heading><p>Stay updated on our latest arrivals, exclusive promotions and events.</p><div className="kaf-footer-socials">{socials.map((social) => <a key={`${social.name}-${social.url}`} href={social.url} target={social.url === "#" ? undefined : "_blank"} rel={social.url === "#" ? undefined : "noreferrer"} aria-label={social.name} title={social.name} style={{ color: social.color }}><SocialIcon platform={social.name} /></a>)}</div>{facebook && <a href={facebook.url} target="_blank" rel="noopener noreferrer" className="kaf-facebook-card" aria-label="Visit KAF LifeStyle on Facebook"><i style={{ color: facebook.color }}><SocialIcon platform="Facebook" /></i><strong><span>KAF LifeStyle <em aria-label="Verified">✓</em></span><small>{s.facebookFollowers ? `${s.facebookFollowers} followers` : "Official Facebook page"}{s.facebookFollowing ? ` · ${s.facebookFollowing} following` : ""}</small></strong><b>Follow</b></a>}<div className="kaf-app-badges"><Image src="/images/app-store-badges.jpg" alt="Download KAF LifeStyle on Google Play and the App Store" fill sizes="330px" className="object-contain object-left" /></div></section>
  </div></div><div className="kaf-footer-bottom py-6 border-t border-white/10 bg-[#080909] text-center"><div className="kaf-footer-bottom-inner text-center space-y-1.5 px-4"><p className="text-xs sm:text-sm text-gray-300 font-medium text-center">Every Order is Packed with Care <span className="text-red-500 mx-0.5">♥</span> and Delivered with Trust.</p><p className="text-xs text-gray-400 text-center">{s.copyrightText || `Copyright © ${new Date().getFullYear()} KAF LifeStyle. All Rights Reserved.`}</p></div></div></footer>;
}
